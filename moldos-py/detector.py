...
class DetectorPose:
    def __init__(self, pose, classification_model, flip_pose_horizontally=True) -> None:
        self.pose = pose
    ...
    def run(self, image):
        ...
        if self.flip_pose_horizontally:
            self.results = self.pose.process(cv2.flip(image, 1))
        else:
    ...
    def detect(self, display_output=True):
        try:
            ...
            if display_output:
                self.image = cv2.putText(image, f"{body_language_class}  Confidence : {round(body_language_prob[np.argmax(body_language_prob)],2) * 100}%", (100, 100), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA) 
            else:
                self.image = image
        except Exception as e:
            print(e)
    ...
    def draw(self):
        self.image = cv2.flip(self.image, 1)
        mp_drawing.draw_landmarks(self.image, self.results.pose_landmarks, mp_pose.POSE_CONNECTIONS, 
                                 mp_drawing.DrawingSpec(color=(245,117,66), thickness=2, circle_radius=4),
                                 mp_drawing.DrawingSpec(color=(245,66,230), thickness=2, circle_radius=2)
                                 )
        self.image = cv2.flip(self.image, 1)